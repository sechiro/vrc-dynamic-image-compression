...
namespace HatagoWorks.DynamicImageCompression
{
    ...
    public class DRCompressedImageDownloader : UdonSharpBehaviour
    {
        ...
        [Header("Downscale")]
        [Tooltip("Target size for the encoded texture. Exact 2x / 4x downloads are box-averaged in the encoder shader; any other download larger than the target is GPU-resized (bilinear, stretched) to exactly this size before encoding. Downloads at or below the target are encoded unchanged (no upscale). 0 disables. SetTargetSize overrides this per request.")]
        ...
        // GPU-resized copy of the current download (arbitrary downscale to the
        // target size). Alive only while its encode runs.
        private RenderTexture _resizeRenderTexture;
        private bool _resizeIsSrgb;
        ...
        public override void OnImageLoadSuccess(IVRCImageDownload result)
        {
            ...
            // Arbitrary downscale: a download larger than the target that is not
            // an exact 2x / 4x multiple is GPU-resized (stretched) to exactly the
            // target size before encoding. Never upscales: a download at or below
            // the target is encoded at its own size.
            bool useResizedSource = false;
            if (_activeDownscaleDivisor == 1
                && _activeTargetWidth > 0
                && _activeTargetHeight > 0
                && (sourceTexture.width != _activeTargetWidth
                    || sourceTexture.height != _activeTargetHeight)
                && sourceTexture.width >= _activeTargetWidth
                && sourceTexture.height >= _activeTargetHeight)
            {
                if (TryCreateResizedSource(sourceTexture, _activeTargetWidth, _activeTargetHeight))
                {
                    useResizedSource = true;
                    encodeWidth = _activeTargetWidth;
                    encodeHeight = _activeTargetHeight;
                }
                else
                {
                    LogWarning("GPU resize to " + _activeTargetWidth + "x" + _activeTargetHeight
                        + " failed; encoding at the downloaded size.");
                }
            }
            ...
            if (IsMobileAstcBuildTarget())
            {
                ...
                if (useResizedSource)
                {
                    astcEncoder.SetDownscaleDivisor(1);
                    astcEncoder.SetSourceRenderTexture(_resizeRenderTexture, _resizeIsSrgb);
                }
                else
                {
            ...
            else if (IsWindowsBuildTarget())
            {
                ...
                if (useResizedSource)
                {
                    bc7Encoder.SetDownscaleDivisor(1);
                    bc7Encoder.SetSourceRenderTexture(_resizeRenderTexture, _resizeIsSrgb);
                }
                else
                {
        ...
        /// <summary>
        /// GPU-resizes the download to exactly targetWidth x targetHeight
        /// (stretched; aspect handling is the content's concern, matching how the
        /// display quad stretches). Halves in steps while the source is still at
        /// least 2x the target in both dimensions, so the final bilinear tap
        /// never skips source texels. The result is stored in
        /// _resizeRenderTexture until ReleaseResizeRenderTexture.
        /// </summary>
        private bool TryCreateResizedSource(Texture2D source, int resizeWidth, int resizeHeight)
        {
            ReleaseResizeRenderTexture();
            _resizeIsSrgb = source.isDataSRGB;
            RenderTextureReadWrite readWrite = _resizeIsSrgb
            ...
            while (width >= resizeWidth * 2 && height >= resizeHeight * 2)
            {
                width /= 2;
                height /= 2;
                RenderTexture step = new RenderTexture(
                    width, height, 0, RenderTextureFormat.ARGB32, readWrite);
                step.filterMode = FilterMode.Bilinear;
                if (!step.Create())
                {
                    DestroyResizeStep(intermediate);
                    return false;
                }
                VRCGraphics.Blit(current, step);
                DestroyResizeStep(intermediate);
                intermediate = step;
                current = step;
            }

            if (width == resizeWidth && height == resizeHeight && intermediate != null)
            {
                _resizeRenderTexture = intermediate;
                return true;
            }

            RenderTexture result = new RenderTexture(
                resizeWidth, resizeHeight, 0, RenderTextureFormat.ARGB32, readWrite);
            ...
            VRCGraphics.Blit(current, result);
            DestroyResizeStep(intermediate);
            _resizeRenderTexture = result;
            return true;
        }

        private void DestroyResizeStep(RenderTexture step)
        {
            if (step != null)
            {
                step.Release();
        ...
        private void ReleaseResizeRenderTexture()
        {
            if (_resizeRenderTexture != null)
            {
                DestroyResizeStep(_resizeRenderTexture);
